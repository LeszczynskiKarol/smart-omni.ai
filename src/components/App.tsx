...
type AppState = "idle" | "listening" | "processing" | "researching" | "speaking";
...
function renderInline(text: string, sources?: Source[]): ReactNode[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (m[2]) parts.push(<strong key={`b${i++}`} className="font-semibold text-gray-100">{m[2]}</strong>);
    else if (m[3]) parts.push(<em key={`i${i++}`} className="italic text-gray-300">{m[3]}</em>);
    else if (m[4]) {
...
function RenderedMarkdown({ text, sources }: { text: string; sources?: Source[] }) {
  ...
  return (
    <div className="space-y-1">
      {lines.map((line, li) => {
        ...
        if (h3) return <h4 key={li} className="text-sm font-semibold text-gray-200 mt-2">{renderInline(h3[1], sources)}</h4>;
        if (h2) return <h3 key={li} className="text-[15px] font-bold text-gray-100 mt-3">{renderInline(h2[1], sources)}</h3>;
        if (h1) return <h2 key={li} className="text-base font-bold text-white mt-3">{renderInline(h1[1], sources)}</h2>;
        if (bullet) return <div key={li} className="flex gap-2 pl-2"><span className="text-accent/50 shrink-0">•</span><span>{renderInline(bullet[1], sources)}</span></div>;
        if (numbered) return <div key={li} className="flex gap-2 pl-1"><span className="text-gray-500 shrink-0 font-mono text-xs w-4 text-right">{numbered[1]}.</span><span>{renderInline(numbered[2], sources)}</span></div>;
...
export default function App() {
  ...
  return <AppMain onLogout={() => { clearToken(); setAuthed(false); }} />;
}
...
function AppMain({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => { chatEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [log.length, liveStatus]);

  // Global search debounced
  useEffect(() => {
    if (searchQuery.trim().length < 2) { setSearchResults(null); return; }
    const t = setTimeout(async () => {
      try { setSearchResults(await globalSearch(searchQuery.trim())); } catch { setSearchResults(null); }
    }, 400);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { setDetailConv(await getConversation(detailConv.id, detailSearch.trim())); } catch {}
    }, 300);
    return () => clearTimeout(t);
  }, [detailSearch]);

  const addLog = (type: LogEntry["type"], text: string, stats?: Stats, actions?: VoiceResponse["actions"], sources?: Source[], didResearch?: boolean) => {
    setLog((p) => [...p, { id: `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 5)}`, type, text, time: new Date(), stats, actions, sources, didResearch }]);
  };
  ...
  const processText = async (text: string) => {
    ...
    try {
      ...
      try {
        res = await sendVoiceStreaming(text, convId || undefined, model, (msg) => {
          setLiveStatus(msg);
      ...
      addLog("assistant", res.response, res.stats, res.actions, res.sources, res.didResearch);
      ...
      setLiveStatus("");
      if (ttsAvailable) { setState("speaking"); await speak(res.response); }
      setState("idle");
      refreshSidebar();
    } catch (err: any) {
  ...
  const startListening = () => {
    ...
    stopSpeaking(); setTranscript(""); setState("listening");
  ...
  const stopListening = () => { if (recRef.current) { recRef.current.stop(); recRef.current = null; } };
  ...
  const handleSend = () => { const t = input.trim(); if (!t) return; setInput(""); processText(t); };
  const newConv = () => { setConvId(null); setLog([]); setDetailConv(null); setLiveStatus(""); inputRef.current?.focus(); };

  const openDetail = async (id: string) => { try { setDetailConv(await getConversation(id)); setDetailSearch(""); } catch {} };

  const continueConv = async (id: string) => {
    try {
      const d = await getConversation(id);
      setConvId(d.id); setDetailConv(null);
      setLog(d.messages.map((m, i) => ({
        id: `l${i}${Date.now().toString(36)}`, type: m.role as any, text: m.content, time: new Date(m.createdAt),
        stats: m.inputTokens != null ? { inputTokens: m.inputTokens!, outputTokens: m.outputTokens!, totalTokens: m.totalTokens!, costUsd: m.costUsd!, model: m.model!, latencyMs: m.latencyMs! } : undefined,
      })));
    } catch {}
  };
  ...
  const saveTopic = async () => {
    ...
    setConversations((p) => p.map((c) => c.id === editingId ? { ...c, topic: editValue.trim() } : c));
    setEditingId(null);
  };
      ...
      <aside className={`${sidebarOpen ? "w-80" : "w-0"} shrink-0 bg-surface-1 border-r border-surface-3 flex flex-col transition-all duration-200 overflow-hidden`}>
        <div className="p-4 border-b border-surface-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={newConv} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-colors">+ Nowa</button>
          </div>
          <div className="relative">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Szukaj we wszystkich..."
            ...
            {searchQuery && <button onClick={() => setSearchQuery("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-sm">✕</button>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {searchResults ? (
            <SearchResultsView results={searchResults} onOpenConv={openDetail} />
          ) : conversations.map((c) => (
            <ConvItem key={c.id} conv={c} active={convId === c.id} editing={editingId === c.id} editValue={editValue}
              onEditChange={setEditValue} onEditSave={saveTopic} onStartEdit={() => { setEditingId(c.id); setEditValue(c.topic); }}
              onClick={() => openDetail(c.id)} onContinue={() => continueConv(c.id)} onDelete={() => handleDelete(c.id)} />
          ))}
        </div>
      </aside>
        ...
        <header className="h-14 px-4 flex items-center justify-between border-b border-surface-3 bg-surface-1/50 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-3">
            ...
            {detailConv ? (
              ...
              <span className="text-sm text-gray-400">{convId ? "Konwersacja aktywna" : "Nowa konwersacja"}</span>
            )}
          </div>
          <div className="flex items-center gap-3">
            {/* Model picker */}
            <div className="flex bg-surface-2 rounded-lg p-0.5">
              {(["claude-haiku-4-5", "claude-sonnet-4-6"] as ModelId[]).map((m) => (
                <button key={m} onClick={() => setModel(m)}
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${model === m ? "bg-accent text-surface-0" : "text-gray-500 hover:text-gray-300"}`}>
                  {m === "claude-haiku-4-5" ? "H" : "S"}
                </button>
              ))}
            </div>
            {!sttAvailable && <span className="text-xs text-yellow-500/70 bg-yellow-500/10 px-2 py-1 rounded">Brak mikrofonu</span>}
            <button onClick={onLogout} className="text-xs text-gray-600 hover:text-red-400 transition-colors" title="Wyloguj">⏻</button>
          </div>
        </header>

        {/* Content */}
        {detailConv ? (
          <DetailView conv={detailConv} search={detailSearch} onSearchChange={setDetailSearch} onContinue={() => continueConv(detailConv.id)} />
        ) : (
          <>
            {/* Chat messages */}
                  ...
                  <div className="animate-fade-in bg-purple-500/10 border border-purple-500/20 rounded-2xl px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-purple-300">
                        {liveStatus || (state === "researching" ? "Analizuję zapytanie..." : "Przetwarzam...")}
                      </span>
                    </div>
                  </div>
                )}
            ...
            <div className="shrink-0 border-t border-surface-3 bg-surface-1/50 backdrop-blur-sm">
              <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
                <button onClick={toggleVoice} disabled={isProcessing || !sttAvailable} className="relative shrink-0 group">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-200 ${
                    state === "listening" ? "bg-red-500/20 text-red-400 ring-2 ring-red-500/40"
                    : state === "speaking" ? "bg-accent/20 text-accent"
                    ...
                    {state === "listening" && <div className="absolute inset-0 rounded-full bg-red-500/20 animate-pulse-ring" />}
                    <span className="text-xl relative z-10">
                      {state === "listening" ? "⏹" : state === "speaking" ? "🔊" : isProcessing ? "🔍" : "🎤"}
                    </span>
                  </div>
                </button>
              ...
              <div className="max-w-3xl mx-auto px-4 pb-2">
                <p className="text-[10px] text-gray-600 font-mono">
                  ...
                  {isProcessing && (liveStatus ? `🔍 ${liveStatus.slice(0, 60)}` : "⏳ przetwarzam...")}
                  {state === "speaking" && "🔊 mówię — kliknij 🎤 aby przerwać"}
                  {state === "idle" && `${model === "claude-sonnet-4-6" ? "Sonnet 4.6" : "Haiku 4.5"} · ${sttAvailable ? "🎤 lub Enter" : "Enter"}`}
                </p>
              </div>
            </div>
          </>
...
function StatMini({ label, value }: { label: string; value: string | number }) {
  return <div><div className="text-xs font-semibold text-gray-200">{value}</div><div className="text-[9px] text-gray-500">{label}</div></div>;
}
...
function EmptyState() {
  return (
    <div className="text-center py-20 animate-fade-in">
      ...
      <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">Głosowy asystent AI z automatycznym researchem online.</p>
      <div className="mt-6 grid grid-cols-2 gap-2 max-w-sm mx-auto text-left">
        {["Utwórz kartę w Trello: naprawić buga", "Jakie są najnowsze modele Claude?", "Porównaj React vs Vue w 2026", "Co mam w kalendarzu?"].map((ex) => (
          <div key={ex} className="text-xs text-gray-500 bg-surface-2 rounded-lg px-3 py-2 border border-surface-3">„{ex}"</div>
        ))}
      </div>
    </div>
  );
  ...
  if (entry.type === "research-status") {
    ...
    return (
      <div className="animate-slide-up bg-purple-500/10 border border-purple-500/20 rounded-2xl px-4 py-3 text-xs space-y-1">
        <div className="text-purple-400 font-medium mb-1">🔍 Proces wyszukiwania</div>
        {lines.map((l, i) => <div key={i} className="text-purple-300/80">{l}</div>)}
      </div>
    );
  }
  ...
  if (entry.type === "action" || entry.type === "error") {
    return (
      <div className={`animate-slide-up text-xs font-mono px-3 py-1.5 rounded-lg ${
        entry.type === "error" ? "bg-red-500/10 text-red-400" : "bg-emerald-500/10 text-emerald-400"
      }`}>
        {entry.text}
      </div>
    );
    ...
    <div className={`animate-slide-up flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        ...
        className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
          isUser ? "bg-accent/15 text-gray-100 rounded-br-md" : "bg-surface-2 text-gray-200 rounded-bl-md border border-surface-3"
        } ${entry.stats ? "cursor-pointer" : ""}`}
      >
        {entry.didResearch && !isUser && (
          <div className="text-[10px] text-purple-400 font-medium mb-1">🔍 Odpowiedź z researchu</div>
        )}
        ...
        {entry.sources && entry.sources.length > 0 && (
          <div className="mt-3 pt-2 border-t border-surface-4">
            <div className="text-[10px] text-gray-500 font-medium mb-1.5">📚 Źródła</div>
            {entry.sources.map((s) => (
              <a key={s.index} href={s.url} target="_blank" rel="noopener"
                ...
                <span className="text-accent font-bold shrink-0">[{s.index}]</span>
                <span className="text-gray-400 truncate group-hover:text-gray-200 transition-colors">{s.title}</span>
        ...
        <div className="flex items-center justify-end gap-2 mt-1">
          ...
          {entry.stats && <span className="text-[10px] font-mono text-gray-600">${entry.stats.costUsd.toFixed(5)}</span>}
        </div>
      </div>
    </div>
  );
...
function ConvItem({ conv, active, editing, editValue, onEditChange, onEditSave, onStartEdit, onClick, onContinue, onDelete }: {
  conv: ConversationSummary; active: boolean; editing: boolean; editValue: string;
  onEditChange: (v: string) => void; onEditSave: () => void; onStartEdit: () => void;
  onClick: () => void; onContinue: () => void; onDelete: () => void;
}) {
  return (
    <div className={`px-4 py-3 border-b border-surface-3/50 hover:bg-surface-2/50 transition-colors ${active ? "bg-accent/5 border-l-2 border-l-accent" : ""}`}>
      {editing ? (
        <input value={editValue} onChange={(e) => onEditChange(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onEditSave()} onBlur={onEditSave} autoFocus
          className="w-full bg-surface-0 border border-accent/50 rounded px-2 py-1 text-sm text-white focus:outline-none" />
      ) : (
        <>
          <div className="flex items-center justify-between gap-2">
            <button onClick={onClick} className="text-sm font-medium text-gray-200 hover:text-white truncate text-left flex-1">{conv.topic || "Nowa konwersacja"}</button>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={onStartEdit} className="text-gray-600 hover:text-gray-300 text-xs p-0.5" title="Edytuj">✏️</button>
              <button onClick={onDelete} className="text-gray-600 hover:text-red-400 text-xs p-0.5" title="Usuń">🗑</button>
            </div>
          </div>
          {conv.lastMessage && <p className="text-xs text-gray-500 truncate mt-1">{conv.lastMessage.role === "user" ? "🗣️ " : "🤖 "}{conv.lastMessage.content}</p>}
          <div className="flex items-center justify-between mt-1.5">
            ...
            <button onClick={onContinue} className="text-[10px] text-accent hover:text-accent-glow font-medium">Kontynuuj ▶</button>
          </div>
        </>
      )}
    </div>
...
function SearchResultsView({ results, onOpenConv }: { results: SearchResult; onOpenConv: (id: string) => void }) {
  return (
    <div className="p-3 space-y-4">
      ...
      <div>
        <h3 className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Wiadomości ({results.messageResults.pagination.total})</h3>
        {results.messageResults.messages.map((m) => (
          <button key={m.id} onClick={() => onOpenConv(m.conversationId)} className="w-full text-left px-3 py-2 rounded-lg bg-surface-2 hover:bg-surface-3 mb-1.5 transition-colors">
            ...
            <div className="text-xs text-gray-400 mt-0.5 line-clamp-2">{m.role === "user" ? "🗣️ " : "🤖 "}{m.matchContext}</div>
          </button>
        ))}
        {results.messageResults.messages.length === 0 && <p className="text-xs text-gray-600 text-center py-4">Brak wyników</p>}
      </div>
    </div>
  );
}
...
function DetailView({ conv, search, onSearchChange, onContinue }: {
  conv: ConversationDetail; search: string; onSearchChange: (v: string) => void; onContinue: () => void;
}) {
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="shrink-0 px-4 py-3 border-b border-surface-3 flex items-center justify-between gap-4">
        ...
        <button onClick={onContinue} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-colors">Kontynuuj ▶</button>
      </div>
      <div className="shrink-0 px-4 py-2 border-b border-surface-3">
        <input value={search} onChange={(e) => onSearchChange(e.target.value)} placeholder="Szukaj w tej konwersacji..."
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="max-w-3xl mx-auto space-y-3">
          {conv.messages.map((m) => <DetailBubble key={m.id} msg={m} />)}
          {conv.messages.length === 0 && <p className="text-sm text-gray-600 text-center py-10">Brak wyników</p>}
        </div>
      </div>
    </div>
  );
...
function DetailBubble({ msg }: { msg: Message }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div onClick={() => !isUser && setOpen(!open)}
        ...
        <div className="text-sm leading-relaxed"><RenderedMarkdown text={msg.content} /></div>
        {open && !isUser && (
          <div className="mt-2 pt-2 border-t border-surface-4 text-[10px] font-mono text-gray-500 space-y-0.5">
